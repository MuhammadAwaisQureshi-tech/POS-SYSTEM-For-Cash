...
def _apply_purchase_invoice_set_fields(record: dict, inv: dict, company_index: int) -> dict:
    """Build MongoDB $set fields to attach or replace a purchase invoice on a daily record."""
...
def upload_purchase_invoice(record_id):
    ...
    try:
        ...
        if not record:
            return jsonify({"error": "Daily record not found"}), 404
        set_fields = _apply_purchase_invoice_set_fields(record, inv, company_index)
...
def update_daily_record(record_id):
    ...
    try:
        ...
        if request.mimetype and request.mimetype.startswith("multipart/form-data"):
            ...
            for key in request.files:
                if key.startswith(prefix):
                    ...
                    if suffix.isdigit():
                        idx = int(suffix)
                        try:
                            inv = _parse_invoice_upload(request.files[key])
                        except ValueError as ve:
                            return jsonify({"error": str(ve)}), 400
                        if inv:
                            multi_inv[idx] = inv
            if not multi_inv:
                ...
                if invoice_file_storage is not None:
                    try:
                        inv = _parse_invoice_upload(invoice_file_storage)
                    except ValueError as ve:
                        return jsonify({"error": str(ve)}), 400
                    if inv:
                        multi_inv[company_index] = inv
        else:
        ...
        if not update and not multi_inv:
        ...
        if pcs and isinstance(pcs, list) and len(pcs) > 0:
            ...
            if 0 in multi_inv:
                ...
                row["purchase_invoice_data"] = inv0["purchase_invoice_data"]
        ...
        if multi_inv and not invoice_merged_into_nested:
            for idx in sorted(multi_inv.keys()):
                frag = _apply_purchase_invoice_set_fields(existing, multi_inv[idx], idx)
                frag.pop("updated_at", None)
                all_set = {**all_set, **frag}
